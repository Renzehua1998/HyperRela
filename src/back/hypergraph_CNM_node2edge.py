...
## 返回的新分区相比旧分区模块度的增量
def addEdge(A, s, edges, m, D, node_P, node_E):
    vol = sum(D)
    M = sum(m)
    ...
    coEdges = set()
    for i in s:
        coEdges = coEdges.union(node_E[i])
    for i in coEdges:
        if edges[i].issubset(s):
            r += 1
    # for e in edges:
    #     if e.issubset(s):
    #         r += 1
    deltaQ = r / M + S
    T4 = time.time()
    print('     边贡献计算时间:%s微秒' % ((T4 - T3)*1000000))
    return deltaQ
...
def cnmAlgo(H, verbose=False):
    ...
    node_E = {i: set() for i in range(n)}  # 每个节点上面有哪些超边
    ...
    e_deltaQ = [0]*len(E)
    for i in range(len(E)):
        e_deltaQ[i] = addEdge(A_opt, E[i], E, m, D, node_P, node_E)

    ## 遍历所有超边，每次放入一个最大模块度提升的超边
    T2 = time.time()
    # print('初始化时间:%s毫秒' % ((T2 - T1)*1000))
    while len(E)>0: 
        e0 = -1
        deltaQ_opt = -1
