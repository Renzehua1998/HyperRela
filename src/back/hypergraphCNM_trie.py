...
## 返回的新分区相比旧分区模块度的增量
def addEdge(A, s, edges, m, D, node_P, edgeTrie):
    vol = sum(D)
    M = sum(m)
    # 统计加入新边后改变的分区
    T1 = time.time()
...
def cnmAlgo(H, verbose=False):
    ...
    for i in range(len(E)):
        e_deltaQ[i] = addEdge(A_opt, E[i], E, m, D, node_P, edgeTrie)
    ...
    while len(E)>0: 
        ...
        if(deltaQ_opt > 0):
            ...
            for i in range(len(r) - 1, -1, -1): 
                # 分别清除超边列表、超边模块度变化列表和超边字典树
                edgeTrie.delete(E[r[i]])
                E.pop(r[i])
                e_deltaQ.pop(r[i])
            ## 计算更新过的deltaQ
            for i in range(len(E)):
